...
/****** Object:  StoredProcedure [dbo].[RVF_PRC_CONTROL_REMITOS_NO_CONFORMADOS]    Script Date: 4/12/2021 5:41:20 PM ******/
SET ANSI_NULLS ON
GO
...
DECLARE					@FechaDesde DATETIME		=		DATEADD(d, -1, CAST(GETDATE() AS DATE)),		-- Ultimos 7 dias
...
SELECT				top 10 NumeroRemito, JobID, FechaConforme, FechaActualizacion
FROM				[CORPSQLMULT01].RV_Remitos.dbo.SyncEpicor							WITH (NoLock)	
WHERE				FechaActualizacion			BETWEEN			DATEADD(d, -1, CAST(GETDATE() AS DATE))		AND		CAST(GETDATE() AS DATE) 
