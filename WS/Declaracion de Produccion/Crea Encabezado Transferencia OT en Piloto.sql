USE [RVF_Local]
DECLARE
			@JobNum		VARCHAR(100)		=			'RV000029',--'RV295001',--'RV213030'-- Sin declarara'RV000014'
			@Declarado	bit					=			0
...
FROM
								...
								(
									...
									******************************************************************/
									...
									FROM				[CORPLAB-EPIDB].[EpicorERP].Erp.JobMtl				JM			WITH (NoLock)
		
									---------------------------------------------------------------------------------------------------------------

									RIGHT  JOIN	[CORPLAB-EPIDB].[EpicorERP].Erp.JobOpDtl				JO			WITH (NoLock)				
									...
									RIGHT  JOIN	[CORPLAB-EPIDB].[EpicorERP].Erp.JobAsmbl				JA			WITH (NoLock)
									...
									RIGHT  JOIN	[CORPLAB-EPIDB].[EpicorERP].Erp.JobHead				JH			WITH (NoLock)
...
LEFT  JOIN		(
					...
					FROM		[CORPLAB-EPIDB].[EpicorERP].Erp.LaborDtl			WITH (NoLock)
					WHERE		jobnum		 =			@JobNum
...
--					FROM 			[CORPLAB-DB-01].[SIP].[dbo].[Labels]					WITH (NoLock)
--					WHERE			OT			=			@JobNum
