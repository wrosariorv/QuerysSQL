...
/****** Object:  StoredProcedure [dbo].[PRC_DESP_SELECT_OV_PLANTA_EMPRESA]    Script Date: 9/12/2025 11:58:43 ******/
SET ANSI_NULLS ON
GO
	...
	IF EXISTS(
				...
				FROM (
						...
						SELECT
									...
									MIN(B1.Plant)  OVER (
										PARTITION BY B1.Company, B1.OrderNum,
													 ISNULL(B1.AnchorLine, B1.OrderLine)
									) AS MinPlantGrp,
									MAX(B1.Plant)  OVER (
										PARTITION BY B1.Company, B1.OrderNum,
													 ISNULL(B1.AnchorLine, B1.OrderLine)
				...
				WHERE   B2.Company   = @Company
				  ...
				  AND (		
							B2.MinPlantGrp  <> B2.MaxPlantGrp
						OR 
							ISNULL(B2.MinPlantaGrp,'') <> ISNULL(B2.MaxPlantaGrp,'')
